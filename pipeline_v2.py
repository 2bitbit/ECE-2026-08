#!/usr/bin/env python3
"""
PROJECT 1 - Edge Vision and Model Quantisation (Optimized)
**********************************************************
...
What it does, in order:
  ...
  3. Trains a WIDER float32 convolutional classifier — conv channels doubled
     (16→32, 32→64) for ~240k parameters (still < 500k) — and reports
     per-epoch validation accuracy, to A/B whether extra capacity actually
     beats the 115k baseline or just overfits the small training set.
  ...
  5. Benchmarks latency single-threaded, batch of one: median and p99 
     (with ORT_ENABLE_ALL graph optimization enabled).
...
Quantisation decisions (and why):
  ...
  * weights          : per-tensor QInt8 (symmetric). A/B-measured against
                       per-channel: accuracy difference is within noise (≈2 images), 
                       but per-tensor reduces memory access overhead for faster inference.
  * activations      : QUInt8 (asymmetric). ReLU outputs are >= 0, so an
                       unsigned, asymmetric range wastes nothing below zero.
  * format (speedup) : QOperator. Fuses nodes into QLinearConv to eliminate Q/DQ 
                       conversion overhead, maximizing integer math efficiency.
"""
...
from starter_p1 import CLASSES, IMG, task1_to_array, synthetic, _find
...
# v2 writes every product into its own folder so running it never clobbers
# pipeline.py's model_*.onnx / results_table.* / figures/.
OUT_DIR = HERE / "v2_output"
...
# 2. model + train
# ***************************************************************************
def task4_model_wide():
    """Wider A/B variant of the baseline: conv channels doubled (16→32, 32→64).
    Parameter count ≈ 240k (still < 500k). Same depth and pooling as the
    baseline, so the only variable is capacity — used to test whether more
    parameters raise test accuracy or just overfit the small training set."""
    n_features = 64 * (IMG // 4) * (IMG // 4)
    layers = [
...
def benchmark_onnx(path, x):
    ...
    # OPTIMIZATION: Enable all graph optimizations to simulate production deployment
...
# 5. quantisation
# ***************************************************************************
def quantize_int8(fp32_path, int8_path, calib_X, per_channel=False,
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # OPTIMIZATION: quant_format=QuantFormat.QOperator added for operator fusion
        # OPTIMIZATION: per_channel=False used based on parameter passing
        quantize_static(
...
# ***************************************************************************
# figures (unchanged layout, style)
# ***************************************************************************
...
def write_results(rows, per_class, meta):
    ...
    (OUT_DIR / "results_meta.json").write_text(json.dumps(meta, indent=2))
...
def main(epochs=20):
    ...
    print("*" * 60)
    print("  PROJECT 1  -  full pipeline (OPTIMIZED)")
    print("*" * 60)

    X, y = load_dataset(ROOT)
    synthetic_used = X is None
    if synthetic_used:
        print("NEU-DET not found -> using synthetic images.\n")
        X, y = synthetic()
    else:
    ...
    # OPTIMIZATION: per_channel changed to False
    quantize_int8(fp32_onnx, int8_onnx, calib_X, per_channel=False,
                  calib_method="percentile")
